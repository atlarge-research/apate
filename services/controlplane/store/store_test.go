...
func TestRemoveNode(t *testing.T) {
	...
	selector := "1231415"
	store := NewStore()
	node := NewNode(*service.NewConnectionInfo("yeet", 42), &scenario.NodeResources{}, selector)
...
func TestGetNodesBySelector(t *testing.T) {
	...
	store := NewStore()
	node := NewNode(*service.NewConnectionInfo("yeet", 42), &scenario.NodeResources{UUID: uuid.New()}, selector)
